...
### Writer's Note
I want to acknowledge that my earlier example in this article was not a strong one. In hindsight, it was more of a frame misinterpretation than a clean hallucination case, and I should have been more careful when I first wrote it. I apologize for that.

A better-known hallucination pattern from earlier-generation LLMs is the generation of non-existent academic papers that still look real (plausible titles, venues, and DOI-like strings). The underlying mechanism is similar: the model completes a statistically likely format, but that completion is not guaranteed to be evidence-true.

Thank you for reading, I appreciate the thoughtful feedback and the opportunity to clarify this point.
